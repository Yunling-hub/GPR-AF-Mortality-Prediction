...
to_winsor <- c("WBC","RBC","Platelets","Hemoglobin","RDW","Hematocrit",
               ...
               "HR","SBP","DBP","RR","Spo2","Temperature")  # no Mean_BP

set.seed(42)
for (v in to_winsor) {
  if (v %in% names(df1)) {
    if (sum(!is.na(df1[[v]])) > 50) df1[[v]] <- winsor(df1[[v]])
  }
}
...
# ----- save -----
dir.create("./data/processed/NWICU/", recursive=T, showWarnings=F)
write.csv(df_final, "./data/processed/NWICU/df_nwicu_cleaned.csv", row.names=F)
saveRDS(df_final, "./data/processed/NWICU/df_nwicu_cleaned.rds")

cat("Done.", nrow(df_final), "rows\n")